...
jQuery(document).ready(function($) {
	...
	$(".mmenu-nav__link").click(function(e) {
		if ($(this).parent(".is-parent").length) {
			e.preventDefault();
			$(this)
				.next(".mmenu-nav__sublevel")
				.addClass("is-active")
				.parent()
				.closest(".mmenu-nav__level")
	...
	// $('.carousel-content').fadeOut(400);
	// var currentModal;
	// var dataValue = $(".carousel-content").attr("data-name");

	// // Opens modal to correct slide when clicked
	//$('.icons-wrapper > .icons').on('click', function(e) {
	//     $('.carousel-content.current').removeClass('current');
	//     $('#risk-carousel-' + $(this).data('modal') + ' [data-slide="' + $(this).data('modal-slide') + '"]').addClass('current');
	//     $('#risk-carousel-' + $(this).data('modal')).removeClass('hide');
	//     currentModal = $('#risk-carousel-' + $(this).data('modal'));
	//     currentModal.find('.carousel-content.current').fadeIn();
	//     onModalSlideChange();
	//     $('body').addClass('no-scroll');
	// });
	...
	// jQuery('.hb-carousel').each(function() {
	//     var numSlides = $(this).find(".carousel-content-wrapper > .carousel-content").length;
	//     var addId = numSlides;
	...
	// $(".hb-carousel .carousel-dots > .dots .dot:first").addClass('active');

	// // Change slides when you click a dot
	// $(".hb-carousel .dot").click(function() {
	...
	//     currentModal.find('.current').fadeOut(500, function() {
	//         currentModal.find('.current').removeClass('current');
	//         next.addClass('current');
	//         onModalSlideChange();
	//         currentModal.find('.current').fadeIn(500);
	//     });
	// });

	// // Changes current slide when an arrow is clicked
	// $(".hb-carousel .nav-arrows > div").click(function() {
	//     if ($(this).hasClass('right')) {
	//         nextSlide = (currentModal.find('.current').next('.carousel-content').length != 0) ? currentModal.find('.current').next('.carousel-content') : currentModal.find('.carousel-content-wrapper .carousel-content:first');
	//     } else {
	//         nextSlide = (currentModal.find('.current').prev('.carousel-content').length != 0) ? currentModal.find('.current').prev('.carousel-content') : currentModal.find('.carousel-content-wrapper .carousel-content:last');
	//     }
	//     currentModal.find('.current').fadeOut(500, function() {
	//         currentModal.find('.current').removeClass('current');
	...
	//     $('.dots .dot:nth-child(' + currentModal.find('.current').data('slide') + ')').addClass('active');

	//     next = (currentModal.find('.current').next('.carousel-content').length != 0) ? currentModal.find('.current').next('.carousel-content') : currentModal.find('.carousel-content-wrapper').find('.carousel-content:first');
	//     currentModal.find('.nav-arrows .right p').text(next.data('name'));

	//     prev = (currentModal.find('.current').prev('.carousel-content').length != 0) ? currentModal.find('.current').prev('.carousel-content') : currentModal.find('.carousel-content-wrapper').find('.carousel-content:last');
	//     currentModal.find('.nav-arrows .left p').text(prev.data('name'));
	// }
	...
	$(".full-width-slider").each(function() {
		...
		for (var i = 0; i < numSlides; i++) {
			$(this)
				.find(".carousel-dots > .dots")
				.prepend('<li class="dot" id="carousel-' + addId + '"></li>');
			var addId = addId - 1;
		}
	});

	$(".full-width-slider .fw-carousel-content-wrapper")
		.not(".current")
		.fadeOut(1);
	...
	$(".full-width-slider .nav-arrows > div").click(function() {
		if ($(this).hasClass("right")) {
			nextSlide =
				$(".full-width-slider .current").next(".fw-carousel-content-wrapper")
					.length != 0
					? $(".full-width-slider .current").next(
							".fw-carousel-content-wrapper"
					  )
					: $(".full-width-slider .fw-carousel-content-wrapper:first");
		} else {
			nextSlide =
				$(".full-width-slider .current").prev(".fw-carousel-content-wrapper")
					.length != 0
					? $(".full-width-slider .current").prev(
							".fw-carousel-content-wrapper"
					  )
					: $(".full-width-slider .fw-carousel-content-wrapper:last");
		}
	...
	function onSlideChange() {
		...
		$(
			".dots .dot:nth-child(" +
				$(".full-width-slider .current").data("slide") +
				")"
		).addClass("active");
		next =
			$(".full-width-slider .current").next(".fw-carousel-content-wrapper")
	...
	$(".flickity-prev-next-button.next").append(
		'<span class="hide-small">Next</span>'
	);
	$(".flickity-prev-next-button.previous").append(
		'<span  class="hide-small">Previous</span>'
	);
	...
	$(".icons-wrapper > .icons").on("click", function(e) {
		...
		$(".hb-carousel .carousel-content-wrapper").flickity(
			"select",
			$(this).data("modal-slide") - 1,
			true,
			true
		);
		onModalSlideChange();
		if (!$("body").hasClass("en-home") && !$("body").hasClass("fr-home")) {
			$("body").addClass("no-scroll");
		}
	});
	...
	function onModalSlideChange() {
		next =
			currentModal.find(".is-selected").next(".carousel-content").length != 0
				? currentModal.find(".is-selected").next(".carousel-content")
				: currentModal
						.find(".carousel-content-wrapper")
						.find(".carousel-content:first");
		currentModal
			.find(".flickity-prev-next-button.next span")
			.text(next.data("name"));

		prev =
			currentModal.find(".is-selected").prev(".carousel-content").length != 0
				? currentModal.find(".is-selected").prev(".carousel-content")
				: currentModal
						.find(".carousel-content-wrapper")
						.find(".carousel-content:last");
		currentModal
			.find(".flickity-prev-next-button.previous span")
			.text(prev.data("name"));
	}
	...
	if ($(".full-width-slider").length != 0) {
		...
		onModalSlideChange();
		$(".full-width-slider .carousel-content-wrapper").on(
			"change.flickity",
			function() {
				onModalSlideChange();
			}
	...
	$("#contactForm").on("submit", function(e) {
		...
		// send to formspree
		$.ajax({
            url: "s//formspree.io/glen@hcansourcing.com",
			method: "POST",
			data: {
				name: name,
				_replyto: email,
